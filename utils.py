...
# Системни роли за достъп (Новата твърда йерархия)
SYSTEM_ROLES = ["Четец", "Power User", "Администратор", "Супер-админ"]
...
# --- МАТРИЦА НА ПРАВАТА (ИНФОРМАТИВНА) ---
...
def check_permission(module, action):
    """
    Твърда йерархична проверка на правата:
    Четец < Power User < Администратор < Супер-админ
    """
    role = st.session_state.get('user_role', '').strip()

    # 1. Бог режим: Супер-админ може абсолютно всичко навсякъде
    if role == "Супер-админ":
        return True

    # Установяване на йерархията (вкл. синоними за съвместимост със стари записи)
    is_admin = role in ["Администратор", "Админ"]
    is_power = role in ["Power User", "Power user", "Редактор"] or is_admin
    is_reader = role in ["Четец", "Супер-четец", "Служител", "AI-User"] or is_power

    # Ако ролята е напълно неизвестна, режем достъпа
    if not is_reader:
        return False

    # --- ПРАВА ЗА МОДУЛ: ПРОПУСНАТИ ПОЛЗИ (MP) ---
    if module == "mp_dashboard":
        if action == "read": return is_reader
        if action == "export": return is_power
        if action == "upload_data": return is_admin
        return False

    # --- ПРАВА ЗА МОДУЛ: СИГНАЛИ (RO/SIGNAL) ---
    if module == "ro_registry":
        if action == "read": return is_reader
        if action == "edit_kanban": return is_power
        if action == "export": return is_power
        if action == "create_ticket": return is_admin
        if action == "cancel_ticket": return is_admin
        if action == "hard_delete": return False  # Изрично забранено (само Супер-админ може)
        return False

    # (Опционално: Права за Рекрутмънт, ако старият модул все още се ползва)
    if module == "recruitment":
        if action == "read": return is_reader
        if action in ["manage_positions", "upload_candidates", "evaluate", "schedule"]: return is_power
        if action == "soft_delete": return is_admin
